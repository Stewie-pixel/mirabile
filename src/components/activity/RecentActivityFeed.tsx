...
export function RecentActivityFeed({ events, loading, compact = false }: RecentActivityFeedProps) {
  ...
  const getEventColor = (type: EventType) => {
    switch (type) {
      ...
      default: return '#00F0FF'; // Themed Cyan
    }
  };
  ...
  if (events.length === 0) {
    return (
      <div className="space-y-4">
        {!compact && <h2 className="text-2xl font-bold text-white mb-8">Recent Activity</h2>}
        <Card className="border border-[#00F0FF]/20 bg-[#00F0FF]/5 rounded-2xl p-8 text-center backdrop-blur-md">
          <Calendar className="w-12 h-12 text-[#00F0FF]/40 mx-auto mb-4" />
    ...
    return (
      <div className="space-y-4">
        ...
        {displayCount < events.length && (
          <Button 
            variant="ghost" 
            className="w-full text-xs text-[#00F0FF] hover:text-[#00F0FF]/80 hover:bg-[#00F0FF]/5 rounded-xl py-2" 
    ...
    <div className="space-y-8">
          ...
          {visibleEvents.map((event, index) => {
            ...
            return (
              ...
              >
                ...
                {/* Card container */}
                <Card 
                  className="w-full md:w-[calc(50%-1.75rem)] border bg-slate-900/40 backdrop-blur-md rounded-2xl p-5 hover:border-[#00F0FF]/35 hover:shadow-[0_4px_24px_rgba(0,240,255,0.08)] transition-all duration-300"
      ...
      {displayCount < events.length && (
        <div className="flex justify-center pt-6">
          <Button 
            className="px-8 py-5 rounded-full text-sm font-semibold text-white bg-gradient-to-r from-[#00F0FF] to-[#F472B6] hover:opacity-90 shadow-lg shadow-[#00F0FF]/15 hover:shadow-[#00F0FF]/25 transition-all duration-300 hover:-translate-y-0.5 active:translate-y-0" 
            onClick={() => setDisplayCount(prev => prev + 5)}
          >
            Load more
          </Button>
